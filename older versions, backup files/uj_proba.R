...
key_word_loop <- function(kw) 
{
  ...
  for (w in 1:length(kw) ) 
  {
    ...
    for (k in 1:2) {
      ...
      colnames(gt_df)[2] <- as.character( kw[w] )
  ...
  out <- list(train_returns, test_returns)
  return(out)
}
...
key_words <- c("debt", "crisis")

key_word_loop(key_words)   



setHandleParameters(user = "hunter5", password = "qwert", domain = "mydomain", proxyhost = "36.89.243.187", proxyport = 42319, proxyauth = 15)

# Start with a fresh handle
h <- new_handle()

# Ask server to set some cookies
req <- curl::curl_fetch_memory("https://eu.httpbin.org/cookies/set?foo=123&bar=ftw", handle = h)
