...
# data_frame <- read.csv(file= "temp.csv", header=TRUE, sep=",") %>% select(1:3)
# data_frame$interest <- na.approx(data_frame$interest)
# limit_a = 40
# limit_c = 60
# #strategy(data_frame, limit_a, limit_c) 
# temp_df <- data_frame
# temp_df <- mutate( temp_df , interest = ifelse(interest == 0, NA, interest) ) #dealing with the 0 interests
# temp_df$interest <- na.approx(temp_df$interest)
...
security <- read.csv(file= "DJI.csv", header=TRUE, sep=",") %>% select(Date, Adj.Close)
...
summary_df <- data.frame(matrix(NA, nrow = 6, ncol = 20))
...
write.csv(summary_df, file = "SUMMARY_DF_FINAL.csv", row.names=FALSE)
