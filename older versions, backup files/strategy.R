...
for (training_from in 1:2) { #training_from=1 from 2004-01-01;    training_from=2 from 2008-08-06
...
file_name <- paste(as.character(training_from), as.character(sd_num), "NEGATIVEEEEEEE_SUMMARY_DF_FINAL.csv", sep="_")
...
if (sd_num==1) {
  elso_output<-summary_df
} else {
  masodik_output<-summary_df
}
